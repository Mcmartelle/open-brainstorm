...
export const template = (ctx, html) => {
  ...
  if (!ctx.createRoomAttempted && !ctx.joinRoomAttempted) {
    ...
    return html`
      ${header}
      <h2 class="room-info"><span class="room-label">ROOM NUMBER:</span><span class='room-id'>${ctx.roomName}</span><button class="leave-room" @click="${(e) => ctx.leaveRoom(e)}">Leave Room</button></h2>
      <ul>
        ${ctx.sortedIdeas.map(
  (idea) => html`
            <li class="idea">
